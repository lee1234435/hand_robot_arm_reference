    # 내가 만든 기능
    # def motion_dance_ff(self):  # designed 'bang bang spin'
    #     try:
    #         self.clientSocket.send('dance_ff_start'.encode('utf-8'))
    #     except:
    #         pass
    ...
    #     self._angle_speed3 = 60
    #     self._angle_acc3 = 300
    #     code = self._arm.set_servo_angle(angle=[179.2, -42.1, 7.4, 186.7, 41.5, -1.6], speed=self._angle_speed3,
    #                                     mvacc=self._angle_acc3, wait=True, radius=0.0)
    #     if not self._check_code(code, 'set_servo_angle'):
    #         return
    ...
    #         if not self.is_alive:
    #             break
    #         code = self._arm.set_servo_angle(angle=[212.0, -21.0, 112.0, 207.0, -0.8, 7.3], speed=self._angle_speed3,
    #                                         mvacc=self._angle_acc3, wait=True, radius=0.0)
    #         if not self._check_code(code, 'set_servo_angle'):
    #             return
    #         code = self._arm.set_servo_angle(angle=[212.0, -38.0, 100.3, 180.4, -6.4, 6.0], speed=self._angle_speed3,
    #                                         mvacc=self._angle_acc3, wait=True, radius=0.0)
    #         if not self._check_code(code, 'set_servo_angle'):
    #             return

    #     self._angle_speed4 = 60
    #     self._angle_acc4 = 200
    #     code = self._arm.set_servo_angle(angle=[179.2, -42.1, 7.4, 186.7, 41.5, -1.6], speed=self._angle_speed4,
    #                                     mvacc=self._angle_acc4, wait=True, radius=0.0)
    #     if not self._check_code(code, 'set_servo_angle'):
    #         return
...
# # MediaPipe
# this_action = '?'
# actions = ['banana', 'choco', 'strawberry']
# seq_length = 30
# model = load_model('gesture-recognition/models/model2_1.0.keras')
# seq = []
# action_seq = []
...
#     else:
#         err = "empty"
#         cv2.putText(img,err,(200,200),cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255),2)
#         this_action = '?'
#         action_seq.clear()
#         seq.clear()
...
#     if this_action != '?':
#         if this_action == 'banana':
#             print("A")

        
#         if this_action == 'choco':
#             print("B")


#         if this_action == 'strawberry':
#             print("C")
...
class RobotMain(object):
                ...
                if self.state == 'icecreaming':
                    ...
                    while True:
                        if self.order_msg['makeReq']['latency'] in ['go', 'stop']:
                            break
                        time.sleep(0.2)
                    if self.order_msg['makeReq']['latency'] == 'go':
                        self.motion_grab_capsule()
                        if self.order_msg['makeReq']['sealing'] == 'yes':
                            ...
                            while True:
                                ...
                                time.sleep(0.2)
                                count += 0.2
                        if self.order_msg['makeReq']['sealing'] == 'go' or self.order_msg['makeReq']['sealing'] not in ['yes', 'stop']:
    ...
    def _get_forward_kinematics__(self, angles, input_is_radian=None, return_is_radian=None):
...
position : -46.6 -161.7 286.6 62.5 84.6 -28.3
angle : 245.6, -27.1, 7.6, 152.9, 60.9, 19.8
